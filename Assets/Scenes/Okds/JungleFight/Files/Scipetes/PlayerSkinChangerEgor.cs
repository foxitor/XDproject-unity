...
public class PlayerSkinChangerEgor : MonoBehaviour {
    ...
    public void Boop() {
        if (Achived) {
            transform.parent.GetChild(0).GetComponent<PlayerSkinChangerEgor>().InUse = false;
            transform.parent.GetChild(1).GetComponent<PlayerSkinChangerEgor>().InUse = false;
            InUse = !InUse;
        }
    }
    public void StartsGame() {
        if (InUse) { PlayerPrefs.SetInt(PPrefName + "LastUsed", 1); 
        GameObject plr = GameObject.Find("Fight/Player").gameObject; 
